...
import BlogBody from "../_component/blog-body";
import BlogHeader from "../_component/blog-header";
import { Heading5 } from "@/components/typography/Heading5";
import BlogBanner from "../_component/blog-banner";
import TableOfContent from "../_component/blog-toc";
import BlogRecent from "../_component/blog-recent";

export async function generateStaticParams() {
  const query = "*[_type == $type]{ slug }";
  const params = { type: "post-mnlbet" };
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title: currentPost.title || "Blog",
      description: currentPost.description || "",
      url: `${process.env.APP_DOMAIN}/news/${slug}`,
    ...
    alternates: {
      cannonical: `${process.env.APP_DOMAIN}/news/${slug}`,
    },
  };
}
...
export default async function ShowNews({
  ...
  return (
    <section className="w-full">
      {/* Header */}
      <header className="p-4 md:p-6 my-2 md:my-4 bg-[radial-gradient(circle_at_center,_#171717_0%,_#1F1F1F_100%)] rounded-[7px] shadow-md">
        <BlogHeader data={currentPost} />
      </header>
        ...
        <div className="lg:order-3 order-4 col-span-1 lg:col-span-1 mb-2">
          <div className="sticky top-42 p-2 md:p-4 bg-[radial-gradient(circle_at_center,_#171717_0%,_#1F1F1F_100%)] rounded-[7px] shadow-md">
            ...
            <div className="space-y-4">
              <BlogRecent otherPost={otherBlogs.slice(0, 2)} />
            </div>
          </div>
        </div>
      </main>
